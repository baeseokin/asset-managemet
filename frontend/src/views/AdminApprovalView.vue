        ...
        <div class="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
          ...
          <div v-else-if="req.request_type === 'modify'" class="space-y-3">
            ...
            <div v-if="getModificationDiff(req).length > 0" class="space-y-2">
              ...
              <div class="overflow-hidden border border-slate-200 rounded-lg text-xs">
                <table class="w-full text-left border-collapse">
                  <thead>
                    <tr class="bg-white border-b border-slate-200 text-[10px] text-slate-400 font-bold">
                      <th class="p-2.5">변경 항목</th>
                      <th class="p-2.5">수정 전</th>
                      <th class="p-2.5">수정 후 (요청)</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-slate-850 font-medium">
                    <tr v-for="diff in getModificationDiff(req)" :key="diff.label" class="hover:bg-slate-100/50">
                      <td class="p-2.5 font-bold text-slate-750">{{ diff.label }}</td>
                      <td class="p-2.5 text-slate-400 line-through">{{ diff.old }}</td>
                      <td class="p-2.5 text-indigo-600 font-bold">{{ diff.new }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
