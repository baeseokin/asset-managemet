...
const qrCodeValue = computed(() => {
  ...
  return `${windowOrigin.value}/home/admin-assets?search=${selectedQrAsset.value.item_code}`
})
...
<template>
  <div class="space-y-6 pb-10 text-slate-800">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold tracking-tight text-slate-900">자산 정보 관리</h1>
        <p class="text-xs text-slate-400 font-semibold mt-1">교회의 자산을 등록하고 정보를 일원화하여 통제합니다.</p>
      </div>
      <div>
      ...
      <div class="space-y-4">
        ...
        <div class="flex flex-wrap items-center gap-3">
          <select 
            ...
            class="input-field py-2 text-xs w-36"
          >
        ...
        <!-- Assets List Table -->
        <div v-if="filteredAssets.length > 0" class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-lg">
          <div class="overflow-x-auto">
            <table class="w-full text-left text-xs border-collapse">
              <thead>
                <tr class="border-b border-slate-200 bg-slate-50 text-slate-400 font-bold">
                  ...
                  <th class="p-4">자산명 (코드)</th>
                  <th class="p-4">카테고리</th>
                  <th class="p-4">시리얼 번호</th>
                  <th class="p-4">보관 위치</th>
                  <th class="p-4">상태</th>
                  <th class="p-4">소속 부서 / 담당자</th>
                  <th class="p-4 text-right">작업</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-850">
                <tr v-for="asset in filteredAssets" :key="asset.id" class="hover:hover:bg-slate-50 transition-all" :class="{ 'bg-slate-100/50': selectedAssetIds.includes(asset.id) }">
                    ...
                    <div class="flex items-center gap-3">
                      ...
                      <div>
                        <div 
                          ...
                          class="font-bold text-slate-800 hover:text-indigo-650 hover:underline cursor-pointer"
                          title="자산 상세 정보 조회"
                        >
                          {{ asset.asset_name }}
                        </div>
                  ...
                  <td class="p-4">
                    <span class="px-2 py-0.5 rounded bg-slate-50 text-slate-350 border border-slate-300">
                      {{ asset.category_name }}
                    </span>
                  </td>
                  <td class="p-4 text-slate-750 font-semibold font-mono">{{ asset.serial_number || '없음' }}</td>
                  <td class="p-4 text-slate-400 font-medium">{{ asset.location || '보관함' }}</td>
                  <td class="p-4">
                    <span :class="getStatusBadgeClass(asset.status)" class="px-2.5 py-0.5 rounded border text-[10px] font-bold">
                      {{ getStatusLabel(asset.status) }}
                    </span>
                  </td>
                  <td class="p-4 text-slate-400">
                    <div class="font-bold">{{ asset.dept_name }}</div>
                    <div class="text-[10px] text-slate-400 font-semibold mt-0.5">{{ asset.manager_name }}</div>
                  </td>
                  <!-- Actions -->
                  <td class="p-4 text-right">
                    <div class="flex justify-end gap-1.5">
                      ...
                      <button 
                        @click="viewAssetDetails(asset)"
                        class="p-2 bg-slate-100 text-slate-750 hover:text-indigo-600 rounded-lg border border-slate-300 hover:bg-slate-100 transition-all"
                        title="자산 상세 정보 조회"
                      >
                        <Eye class="w-4 h-4" />
                      </button>
                      ...
                      <button 
                        ...
                        class="p-2 bg-slate-100 text-slate-750 hover:text-emerald-600 rounded-lg border border-slate-300 hover:bg-slate-100 transition-all"
                        title="QR 코드 라벨 발행"
                      >
                      ...
                      <button 
                        ...
                        class="p-2 bg-slate-100 text-slate-750 hover:text-slate-900 rounded-lg border border-slate-300 hover:bg-slate-100 transition-all"
                        title="자산 정보 수정"
                      >
                        <Edit class="w-4 h-4" />
                      </button>

                      <!-- Delete dropdown / direct button -->
                      <button 
                        @click="deleteAsset(asset.id, 'dispose')"
                        class="p-2 bg-white border border-slate-200 text-slate-400 hover:text-amber-600 hover:bg-amber-50 hover:border-amber-500/20 rounded-lg transition-all"
                        title="자산 폐기 신청"
                      >
                        <Trash class="w-4 h-4" />
                      </button>
                      <button 
                        ...
                        class="p-2 bg-white border border-slate-200 text-slate-400 hover:text-rose-400 hover:bg-rose-50 hover:border-rose-500/20 rounded-lg transition-all"
                        title="자산 삭제 신청"
                      >
    ...
    <Teleport to="body">
      <div v-if="showAssetModal" class="fixed inset-0 bg-slate-50/85 backdrop-blur-sm flex items-center justify-center p-4 z-50">
        <div class="relative w-full max-w-lg bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden text-slate-800">
    ...
    <Teleport to="body">
      <div v-if="showQrModal" class="fixed inset-0 bg-slate-50/85 backdrop-blur-sm flex items-center justify-center p-4 z-50">
        <div class="relative w-full max-w-sm bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden text-slate-800">
    ...
    <Teleport to="body">
      <div v-if="showMaintenanceModal" class="fixed inset-0 bg-slate-50/85 backdrop-blur-sm flex items-center justify-center p-4 z-50">
        <div class="relative w-full max-w-lg bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden text-slate-800">
          <div class="p-5 border-b border-slate-200 flex justify-between items-center">
            <h3 class="text-sm font-bold text-slate-900 flex items-center gap-1.5">
          ...
          <div class="p-6 space-y-5 max-h-[65vh] overflow-y-auto text-xs">
            ...
            <div v-if="selectedMaintenanceAsset" class="p-3 bg-white/40 rounded-xl border border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              ...
              <button 
                ...
                class="btn-secondary py-1.5 px-3 text-[10px] flex items-center gap-1 shrink-0 bg-white hover:bg-amber-50 hover:text-amber-600 hover:border-amber-200 border-slate-200 transition-colors"
              >
            ...
            <form @submit.prevent="addMaintenanceLog" class="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
              ...
              <button type="submit" class="btn-primary w-full py-2 text-xs font-bold shadow-none">완료 처리 및 정비 내역 등록</button>
            </form>
          </div>
    ...
    <Teleport to="body">
      <div v-if="showDetailModal" class="fixed inset-0 bg-slate-50/85 backdrop-blur-sm flex items-center justify-center p-4 z-50">
        <div class="relative w-full max-w-lg bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden text-slate-800">
          <!-- Header -->
          <div class="p-5 border-b border-slate-200 flex justify-between items-center">
