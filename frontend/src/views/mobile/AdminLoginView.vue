    ...
    <div class="relative w-full max-w-md transition-all duration-500">
      ...
      <div class="glass-card bg-white/60 border border-slate-200 p-8 md:p-10 shadow-2xl rounded-2xl">
        ...
        <div class="flex flex-col items-center">
          <router-link to="/m/register" class="group flex items-center gap-1.5 text-sm font-bold text-indigo-600 hover:text-indigo-350 transition-all">
...
const handleLogin = async () => {
  ...
  if (result.success) {
    if (auth.user.mustChangePassword) {
      router.push({ name: 'MobileChangePassword' })
      return
    }

    const redirectPath = route.query.redirect || '/m/home'
