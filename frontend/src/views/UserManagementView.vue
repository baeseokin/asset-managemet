...
<template>
  <div class="space-y-6 pb-10 text-slate-800">
        ...
        <table class="w-full text-left text-xs border-collapse">
          <thead>
            <tr class="border-b border-slate-200 bg-slate-50 text-slate-400 font-bold">
              <th class="p-4">사용자 정보</th>
              <th class="p-4">부서 / 연락처</th>
              <th class="p-4 text-center">권한</th>
              <th class="p-4 text-center">가입상태</th>
              <th class="p-4 text-right">관리 작업</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-850">
            <tr v-for="user in filteredUsers" :key="user.id" class="hover:bg-slate-50/20 transition-all">
              <td class="p-4">
                <div class="flex items-center gap-3">
                  <div class="w-9 h-9 rounded-lg bg-slate-50 text-slate-400 flex items-center justify-center font-black">
                    {{ user.user_name.charAt(0) }}
                  </div>
                  <div>
                    <div class="font-bold text-slate-800">{{ user.user_name }}</div>
              ...
              <td class="p-4">
                <div class="flex flex-col gap-1">
                  <div class="flex items-center gap-1.5 text-slate-350 font-bold">
                    <Building2 class="w-3.5 h-3.5 text-slate-400" />
                    {{ user.dept_name || '부서 미지정' }}
                  </div>
                  <div class="flex items-center gap-1.5 text-slate-400 font-semibold">
                    <Phone class="w-3.5 h-3.5 text-slate-600" />
                    {{ user.phone || '연락처 없음' }}
                  </div>
                </div>
              </td>
              <td class="p-4 text-center">
                <div class="flex justify-center gap-1">
                  <span v-for="role in (user.roles || '').split(',').filter(r => r)" :key="role"
                    :class="role === '관리자' ? 'bg-rose-50 text-rose-400 border-rose-500/20' : 'bg-indigo-500/10 text-indigo-600 border-indigo-200'"
              ...
              <td class="p-4 text-center">
                <span 
                  :class="user.is_approved ? 'bg-emerald-500/10 text-emerald-650 border-emerald-500/20' : 'bg-amber-50 text-amber-600 border-amber-500/20'"
                  class="px-2 py-0.5 rounded border text-[10px] font-bold"
                >
                ...
                <div class="flex justify-end gap-2">
                  <template v-if="!user.is_approved">
                    <button @click="approveUser(user)" title="가입 승인"
                      class="px-2.5 py-1.5 bg-emerald-600/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-650 hover:text-white rounded-lg font-bold flex items-center gap-1 transition-all"
                    >
                  ...
                  <template v-else>
                    <button @click="openEdit(user)" title="정보 및 권한 수정"
                      class="p-2 bg-slate-100 text-slate-350 hover:text-slate-900 rounded-lg border border-slate-300 hover:bg-slate-100 transition-all"
                    >
                      <Edit class="w-3.5 h-3.5" />
                    </button>
                    <button @click="resetPassword(user)" title="비밀번호 초기화 ('asset00!')"
                      class="p-2 bg-amber-50 text-amber-600 border border-amber-500/20 hover:bg-amber-500 hover:text-slate-900 rounded-lg transition-all"
                    >
                      <Key class="w-3.5 h-3.5" />
                    </button>
                  </template>
                  <button @click="deleteUser(user)" title="사용자 삭제"
                    class="p-2 bg-white border border-slate-200 text-slate-400 hover:text-rose-600 hover:bg-rose-50 hover:border-rose-500/20 rounded-lg transition-all"
                  >
    ...
    <!-- Edit User Modal -->
    <Teleport to="body">
      <div v-if="showEditModal" class="fixed inset-0 bg-slate-50/85 backdrop-blur-sm flex items-center justify-center p-4 z-50">
        <div class="relative w-full max-w-md bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden text-slate-800">
          <!-- Modal Header -->
